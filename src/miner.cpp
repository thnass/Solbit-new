...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SolbitMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SolbitMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SolbitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("solbit-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SolbitMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
