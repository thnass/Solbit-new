...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SBT)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_Solbit4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_Solbit, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("Solbit")
    updateDisplayUnit();
}
...
double BlockRewardSolbit =  static_cast<double>(BlockReward/COIN);
//int64_t SolbitSupply = chainActive.Tip()->nMoneySupply / COIN;
...
ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardSolbit));
//ui->label_SolbitSupply_value->setText(QString::number(SolbitSupply));
