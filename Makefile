...
pkgdatadir = $(datadir)/solbit
pkgincludedir = $(includedir)/solbit
pkglibdir = $(libdir)/solbit
pkglibexecdir = $(libexecdir)/solbit
...
CONFIG_HEADER = $(top_builddir)/src/config/solbit-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/solbit-config.h.in \
...
ACLOCAL = ${SHELL} /home/ubuntu/build/solbit/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/ubuntu/build/solbit/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/ubuntu/build/solbit/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/ubuntu/build/solbit/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = solbit-cli
BITCOIN_DAEMON_NAME = solbitd
BITCOIN_GUI_NAME = solbit-qt
BITCOIN_TX_NAME = solbit-tx
...
MAINT = 
MAKEINFO = ${SHELL} /home/ubuntu/build/solbit/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = solbit
PACKAGE_BUGREPORT = www.solbit.net
PACKAGE_NAME = Solbit Core
PACKAGE_STRING = Solbit Core 1.3.0
PACKAGE_TARNAME = solbit
PACKAGE_URL = 
...
PROTOBUF_LIBS = -L/usr/local/lib -lprotobuf -pthread
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/ubuntu/build/solbit
abs_srcdir = /home/ubuntu/build/solbit
abs_top_builddir = /home/ubuntu/build/solbit
abs_top_srcdir = /home/ubuntu/build/solbit
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/ubuntu/build/solbit/build-aux/install-sh
...
src/config/solbit-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/solbit-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/solbit-config.h
$(top_srcdir)/src/config/solbit-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/solbit-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Solbit-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
#block_test.info: test_solbit_filtered.info
#	$(MKDIR_P) qa/tmp
...
#total_coverage.info:  baseline_filtered_combined.info test_solbit_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_solbit_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt
...
clean-local:
	rm -rf test_solbit.coverage/ total.coverage/ $(OSX_APP)
